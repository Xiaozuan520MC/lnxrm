...
| 系统调用 | int 0x80 门（DPL3），31 个系统调用（0-30） |
| 用户态程序 | 每个命令是独立 ELF 二进制（`/bin/*`），通过 fork+execve 运行 |
| 文件系统 | VFS 路由层；ramfs（`/dev/console`）；**FAT32**（读/写/创建，LFN 读） |
...
* **FAT32**：读/写/创建/删除文件和目录，支持长文件名（LFN）
* **64MB 虚拟磁盘**：QEMU 启动时自动挂载 `build/disk.img` 到 `/`
...
已知限制
* 磁盘写入已实现（FAT32 创建/写入/删除），但缺少 journal 保护，断电可能丢数据。
