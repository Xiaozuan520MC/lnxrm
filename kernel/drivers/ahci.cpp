/* ahci.cpp: Simplified AHCI/SATA driver — PIO-mode transfers, same style as ide.c */
#include <ahci.h>
#include <io.h>
#include <mm.h>
#include <console.h>
...
static int portStart(volatile u8 *px)
{
    ...
    for (int i = 0; i < 1000000; i++) {
        cmd = pxRd(px, P_CMD);
        if ((cmd & (PORT_CMD_CR | PORT_CMD_FR)) == (PORT_CMD_CR | PORT_CMD_FR))
            return 0;
        asm volatile("pause");
    }
    return -1;
...
static int issueCmd(volatile u8 *px, int slot)
{
    ...
    for (int i = 0; i < 10000000; i++) {
        if (!(pxRd(px, P_CI) & (1u << slot)))
            return (pxRd(px, P_IS) & IS_TFES) ? -1 : 0;
        asm volatile("pause");
    }
    return -1;
}
...
static int doPioIn(volatile u8 *px, AhciPort *port, u8 cmd, u64 lba,
                    u32 count, void *buf, u32 bytes)
{
    CmdHdr *h = &port->clb[0];
    CmdTbl *t = port->ct;
...
static int doPioOut(volatile u8 *px, AhciPort *port, u8 cmd, u64 lba,
                     u32 count, const void *buf, u32 bytes)
{
    CmdHdr *h = &port->clb[0];
    CmdTbl *t = port->ct;
...
static int ahciBlkRead(struct blkdev *bd, u64 lba, u32 cnt, void *buf)
{
    ...
    for (u32 i = 0; i < cnt; i++) {
        if (doPioIn(port->mmio, port, 0x25, lba + i, 1, p + i * 512, 512))
            return -1;
    }
    return 0;
}
...
static int ahciBlkWrite(struct blkdev *bd, u64 lba, u32 cnt, const void *buf)
{
    ...
    auto *p = (const u8 *)buf;
    for (u32 i = 0; i < cnt; i++) {
        if (doPioOut(port->mmio, port, 0x35, lba + i, 1, p + i * 512, 512))
            return -1;
    }
    u8 dummy[512];
    doPioIn(port->mmio, port, 0x25, lba, 1, dummy, 512);
    return 0;
}
...
static int ahciProbe(void *, u8 bus, u8 dev, u8 fn,
                     u16 vendor, u16 devid, u8 class_)
{
    if (class_ != 1)
        return 0;
    ...
    if (ghc & GHC_HR) {
        *(volatile u32 *)(mmio + AHCI_GHC) = GHC_HR;
        for (int i = 0; i < 1000000; i++) {
            if (!(*(volatile u32 *)(mmio + AHCI_GHC) & GHC_HR))
                break;
            asm volatile("pause");
        }
    }
    ...
    *(volatile u32 *)(mmio + AHCI_GHC) = GHC_ARE;
    ...
    for (int p = 0; p < 32; p++) {
        ...
        if (doPioIn(px, port, 0xEC, 0, 1, ident, 512)) {
            kprintf("[ahci] port %d: IDENTIFY failed tfd=%02x is=%08x\n",
                    p, pxRd(px, P_TFD), pxRd(px, P_IS));
            continue;
        }

        u64 sectors = *(u32 *)&ident[60];
        if (!sectors)
            sectors = *(u32 *)&ident[100];
...
extern "C" int ahci_probe_fn(void *ctx, u8 bus, u8 dev, u8 fn,
                              u16 vendor, u16 devid, u8 class_)
{
