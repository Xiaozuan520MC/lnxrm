...
/* Poll PS/2 controller: drain any pending scancodes from port 0x60.
 * Use when IRQ delivery is unreliable (e.g. nested IF=0 context). */
void kbd_poll(void)
{
    while (inb(KBD_STAT) & 1) {  /* OBF = output buffer full */
        u8 sc = inb(KBD_DATA);
        kbd_process_sc(sc);
    }
}
