...
void start_kernel(struct boot_params *bp)
{
    ...
    if (grub_params_pa) {
        ...
        const struct boot_params *grub_bp =
            (const struct boot_params *)grub_params_pa;
        u32 n = grub_bp->e820_entries;
        if (n > 128)
            n = 128;
        for (u32 i = 0; i < n; i++) {
            if (!grub_bp->e820_map[i].size)
                continue;
     ...
     * setup.asm runs on both the QEMU -kernel and GRUB linux paths,
     * and stores results at physical 0x8C00. */
    ...
    if (bootinfo.fb.ok)
        kprintf("[vbe] mode ok: %dx%d %d bpp phys=0x%x pitch=%d\n",
                bootinfo.fb.width, bootinfo.fb.height, bootinfo.fb.bpp,
                (u32)bootinfo.fb.phys_addr, bootinfo.fb.pitch);
    else
        kprintf("[vbe] no valid VBE mode found\n");
