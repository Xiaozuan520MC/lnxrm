...
{
    /* INIT, level assert, level-triggered (bit 15 = trigger mode) */
    while (apic_read(LAPIC_ICR_LOW) & (1 << 12))
        ;
    apic_write(LAPIC_ICR_HIGH, dest_apic_id << 24);
    apic_write(LAPIC_ICR_LOW, (5 << 8) | (1 << 14) | (1 << 15));
}
...
{
    /* INIT, level deassert, level-triggered, broadcast (shorthand = 11b) */
    while (apic_read(LAPIC_ICR_LOW) & (1 << 12))
        ;
    apic_write(LAPIC_ICR_HIGH, 0);
    apic_write(LAPIC_ICR_LOW, 0 | (5 << 8) | (0 << 14) | (1 << 15) | (3 << 18));
}
