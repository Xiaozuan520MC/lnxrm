...
static int cb_find(const char *name, const struct fat_dirent *e, void *ctx)
{
    ...
    if (!strcasecmp(name, f->want) || name_eq_short(f->want, e)) {
        f->found = *e;
        return 1;
    }
    return 0;
...
static int fat_create(void *mnt, const char *path)
{
    ...
    for (u32 cc = pdir;; cc = fat_next(m, cc)) {
        ...
        for (u32 s = 0; s < m->sectors_per_cluster; s++) {
            ...
            for (u32 doff = 0; doff <= m->bytes_per_sector - DIRENT_SIZE;
                 doff += DIRENT_SIZE) {
                struct fat_dirent *e = (struct fat_dirent *)(cbuf + doff);
                if (e->name[0] == ENT_END || e->name[0] == ENT_FREE ||
                    ...
                    /* file starts with cluster 0 (empty) and size 0 */
                    e->fstcluslo = 0;
                    e->fstclushi = 0;
                    e->filesize = 0;
                    fat32_journal_add(lba, cbuf);
                    wr_sec(m, lba, cbuf);
                    fat32_journal_commit();
                    return 0;
                }
...
static int fat_mkdir(void *mnt, const char *path)
{
    ...
    u32 new_clus = find_free_cluster(m);
    if (!new_clus)
        return -28; /* ENOSPC */
    fat_set(m, new_clus, 0x0FFFFFFF); /* mark end of chain */
    ...
    dotdot->fstcluslo = pdir & 0xFFFF;
    dotdot->fstclushi = pdir >> 16;
    dotdot->filesize = 0;

    /* write the first sector of the new directory cluster */
    u64 new_lba = clus_lba(m, new_clus);
    wr_sec(m, new_lba, dir_buf);

    /* zero remaining sectors of the cluster */
    memset(dir_buf, 0, sizeof(dir_buf));
    for (u32 s = 1; s < m->sectors_per_cluster; s++)
        wr_sec(m, new_lba + s, dir_buf);
        ...
        for (u32 s = 0; s < m->sectors_per_cluster; s++) {
            ...
            for (u32 doff = 0; doff <= m->bytes_per_sector - DIRENT_SIZE;
                ...
                if (e->name[0] == ENT_END || e->name[0] == ENT_FREE ||
                    ...
                    memset(e, 0, DIRENT_SIZE);
                    name_to_short(base, e->name);
                    e->attr = ATTR_DIR | ATTR_ARCHIVE;
                    e->crttime = 0x6000; e->crtdate = 0x5A00;
                    e->wrttime = 0x6000; e->wrtdate = 0x5A00;
                    e->fstcluslo = new_clus & 0xFFFF;
                    e->fstclushi = new_clus >> 16;
                    e->filesize = 0;
    ...
    fat32_journal_abort();
    /* if we get here, no free slot was found — free the cluster */
    fat_set(m, new_clus, 0);
    return -1;
}
...
static int fat_rmdir(void *mnt, const char *path)
{
    ...
    u8 dbuf[512];
    int entry_count = 0;
    for (u32 cc = dir_clus; !clus_is_eoc(cc) && cc >= 2; cc = fat_next(m, cc)) {
        for (u32 s = 0; s < m->sectors_per_cluster; s++) {
            u64 lba = clus_lba(m, cc) + s;
            rd_sec(m, lba, dbuf);
            for (u32 doff = 0; doff <= m->bytes_per_sector - DIRENT_SIZE;
                ...
                if (e->name[0] == ENT_END)
                    goto done_check;
                if (e->name[0] == ENT_FREE || e->name[0] == ENT_E5)
                    continue;
                entry_count++;
                /* . and .. are allowed */
...
done_check:
    ...
    /* free the cluster chain of the directory */
    fat32_journal_begin();
    u32 clus = dir_clus;
