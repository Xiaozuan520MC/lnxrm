/* ramfs: in-memory filesystem with /dev/console char device. */
#include <vfs.h>
#include <console.h>
#include <mm.h>
...
static struct rnode *ramfs_mkdir_node(struct rnode *parent, const char *name)
{
...
static struct rnode *ramfs_create_node(struct rnode *parent, const char *name, int type)
{
    struct rnode *n = kmalloc(sizeof(*n));
    memset(n, 0, sizeof(*n));
    strncpy(n->name, name, sizeof(n->name) - 1);
...
void ramfs_init(void)
{
    ...
    /* create /dev/console char device */
    struct rnode *dev = ramfs_mkdir_node(&root_node, "dev");
    ramfs_create_node(dev, "console", V_CHR);
}
