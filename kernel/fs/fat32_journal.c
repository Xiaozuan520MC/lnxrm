...
int fat32_journal_add(u64 lba, const void *data)
{
    ...
    if (global_journal.count >= MAX_JOURNAL_ENTRIES)
        return -2;  /* Journal full */
...
{
    return global_journal.active;
}

/* Get journal statistics */
void fat32_journal_stats(u32 *entries, bool *active)
{
    if (entries) *entries = global_journal.count;
    if (active) *active = global_journal.active;
}