/* Virtual filesystem: path routing between ramfs (root) and
 * an optional FAT32 mount at /, plus the fd table & syscall surface. */
#include <vfs.h>
#include <console.h>
#include <mm.h>
#include <sched.h>
...
{
    char abuf[128];
    /* normalize: prepend "/" for relative paths */
    if (path[0] != '/') {
        abuf[0] = '/';
        int i;
        for (i = 1; i < (int)sizeof(abuf) - 2 && path[i - 1]; i++)
            abuf[i] = path[i - 1];
        abuf[i] = 0;
        path = abuf;
    }
    ...
    if (!strcmp(path, "/")) {
        /* try disk root first if mounted at "/" */
        if (disk_ready) {
        ...
        if (!kern_text_ptr((u64)ops->lookup))
            return -2;
        if (ops->lookup(mnt, p, &vn) < 0) {
            /* fallback: try ramfs for virtual files (e.g. /dev/console) */
            if (ops != &ramfs_ops) {
                const char *rp = path;
                if (ramfs_ops.lookup(NULL, rp, &vn) == 0)
                    goto resolved;
            }
            if (!(flags & O_CREAT))
                return -2;          /* ENOENT */
...
int vfs_try_mount_disk(void)
{
    ...
    kprintf("[vfs] attempting disk mount on %s (%llu sectors, %llu MB)\n",
            blk_first->name, blk_first->num_sectors, blk_first->num_sectors / 2048);
