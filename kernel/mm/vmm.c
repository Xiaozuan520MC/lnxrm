...
u64 master_pml4_phys(void) { return pml4_phys; }
...
static inline u64 *ptable_ptr(u64 pa) { return (u64 *)PHYS_TO_VIRT(pa); }

static u64 ensure_table(u64 *parent, u64 idx, u64 extra_flags)
{
    if (parent[idx] & PG_P)
        return parent[idx] & ~0xfffUL;
    u64 t = pmm_alloc();
    if (!t)
        panic("vmm: out of frames");
...
{
    if (va < USER_BASE || va >= USER_MAX_VMA || (va & 0xfff))
        return -1;
...
static int walk(u64 root, u64 va, u64 **pte_out)
{
    u64 i4 = (va >> 39) & 511, i3 = (va >> 30) & 511;
    u64 i2 = (va >> 21) & 511, i1 = (va >> 12) & 511;
    u64 *pml4 = ptable_ptr(root);
    ...
    if (pd[i2] & PG_PS) {
        static u64 big;
        big = pd[i2];               /* caller only reads the phys bits */
        *pte_out = &big;
        return 1;                   /* 2 MiB leaf */
    }
...
{
    if (va >= ALIAS_BASE && va < ALIAS_BASE + 0x100000000UL)
        return va - ALIAS_BASE;
    u64 *pte;
    int r = walk(root, va, &pte);
    if (r < 0 || !(*pte & PG_P))
        return 0;
    if (r == 1)                     /* 2 MiB leaf */
...
{
    u64 *pte;
    if (walk(root, va, &pte) != 0 || !(*pte & PG_P))
        return 0;
    u64 pa = *pte & ~0xfffUL;
    *pte = 0;
    __asm__ volatile("invlpg (%0)" ::"r"(va) : "memory");
...
u64 vmm_translate(u64 va)
{
    ...
    return vmm_translate_in(cr3, va);
}
...
void vmm_init(void)
{
    ...
    u64 lo = ALIGN_DOWN(0x1400000UL, 0x200000UL);   /* 20 MiB: past image */
    u64 hi = ALIGN_UP(heap_phys + heap_size, 0x200000UL);

    for (u64 pa = lo; pa < hi && pa < 0xC0000000UL; pa += 0x200000UL) {
        u64 idx = (pa >> 21) & 511;
        if (idx == 96 || idx == 112 || idx == 128)
            continue;               /* VGA / fixmap / device windows */
        pdhi[idx] = pa | PG_P | PG_W | PG_PS;
    }

    /* kernel heap window (KHEAP_VMA -> PD_HI[256..271], 2 MiB pages) */
    for (int i = 0; i < 16; i++)
        pdhi[256 + i] = (heap_phys + i * 0x200000UL) | PG_P | PG_W | PG_PS;

    load_cr3(pml4_phys);
}

/* Map an arbitrary physical page at FIXMAP_VA (kernel context only). */
void fixmap_map(u64 pa)
{
    static bool init;
    if (!init) {
        *(volatile u64 *)(PHYS_TO_VIRT(PD_HI) + 112 * 8) = PT_FIX | PG_P | PG_W;
        memset((u8 *)PHYS_TO_VIRT(PT_FIX), 0, PAGE_SIZE);
        init = true;
    }
    *(volatile u64 *)(PHYS_TO_VIRT(PT_FIX)) = pa | PG_P | PG_W;
...
int dup_user_aspace(u64 src, u64 dst)
{
    ...
    if (!(s4[USER_SLOT] & PG_P))
        return 0;                   /* nothing mapped yet */
    ...
    for (int i3 = 0; i3 < 512; i3++) {
        ...
        for (int i2 = 0; i2 < 512; i2++) {
            ...
            u64 *spt = ptable_ptr(spd[i2] & ~0xfff), *dpt =
                ptable_ptr(d1_pa);

            for (int i1 = 0; i1 < 512; i1++) {
                if (!(spt[i1] & PG_P))
                    continue;
                u64 va = slot | ((u64)i3 << 30) | ((u64)i2 << 21) |
                         ((u64)i1 << 12);
                /* defensive: only copy pages that truly resolve */
                if (!vmm_translate_in(src, va))
                    continue;
                u64 npa = pmm_alloc();
                if (!npa)
                    return -1;
