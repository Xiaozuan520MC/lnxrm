...
.e820_done:
    ...
    ; ---- VBE: query mode info for 640x480x256 and set mode 0x101 --------
    ; Query VBE mode 0x101 info into buffer at 0x8000 (256 bytes).
    ; ModeInfoBlock offset 4 = PhysBasePtr (framebuffer physical address).
    DBG 'V'
    push ds
    xor ax, ax
    ...
    mov cx, 0x0101              ; mode 0x101 = 640x480x8bpp
    mov di, 0x8000              ; buffer at physical 0x8000
    int 0x10
    cmp ax, 0x004F              ; success?
    jne .vbe_pop_fail

    ; Save framebuffer info while DS=0 (flat addressing).
    ; VBE ModeInfoBlock: PhysBasePtr @ offset 0x28, BytesPerScanLine @ 0x10,
    ; XResolution @ 0x12, YResolution @ 0x14, BitsPerPixel @ 0x19.
    mov eax, [0x8028]           ; PhysBasePtr (offset 0x28)
    mov [0x8C00], eax
    movzx eax, word [0x8010]   ; BytesPerScanLine (pitch)
    mov [0x8C04], ax
    ...
    ; Do NOT set VBE mode here — keep VGA in text mode so boot messages
    ; are visible.  fb_init() will switch to graphics mode later via
    ; the Bochs DISPI I/O ports (0x1CE/0x1CF) from protected mode.
    pop ds
    jmp .vbe_done
